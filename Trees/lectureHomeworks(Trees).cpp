// lectureHomeworks(Trees).cpp : Defines the entry point for the console application.
//
...
// how many times the sum of the values ​​of the children(at one level) is equal to the parent ?

int countChilds(Node<int> *root) {
	if (root == nullptr || root->left == nullptr && root->right == nullptr) {
		return 0;
	}
	if (root->left == nullptr) {
		return root->data == root->right->data ? 1 + countChilds(root->right) : countChilds(root->right);
	}
	if (root->right == nullptr) {
		return root->data == root->left->data ? 1 + countChilds(root->left) : countChilds(root->left);
	}
...
void testCountChilds() {
	Node<int> * t = new Node<int>{ 10,
		...
		new Node<int>{ 7,nullptr,nullptr } },
		new Node<int>{ 5,nullptr,nullptr } };
	assert(countChilds(t)==1);
}
// smallest common parent of v1 and v2
// binary ordered tree
...
//Is the value of the father greater than the sum of all his children
bool checkSum(Node<int> *root) {
	if (root == nullptr || (root->left == nullptr && root->right == nullptr)) {
		return true;
	}
	if (root->left == nullptr) {
	...
	if (root->data > root->left->data + root->right->data) {
		return true && checkSum(root->left) && checkSum(root->right);
	}
	return false;
}
...
int main()
{
	Node<int> * t = new Node<int>{ 10,
		...
		new Node<int>{ 5,nullptr,nullptr } };
	std::cout << countChilds(t);
	testCountChilds();
	Node<int> * t2 = new Node<int>{ 11,
		new Node<int>{ 5,
		new Node<int>{ 2,nullptr,nullptr },
		new Node<int>{ 2,nullptr,nullptr } },
		new Node<int>{ 5,nullptr,nullptr } };
	std::cout << checkSum(t2);
	return 0;
}
